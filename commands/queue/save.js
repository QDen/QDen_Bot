...
module.exports = {
    ...
    run: async(bot, message, args)=>{
        ...
        if (bot.queue.length === 0) {
            message.reply(":x: **The queue is empty! There is nothing to save!**")
                    ...
                    let interval;
                        if (args[1]) {
                            ...
                            if (isNaN(check)) {
                                ...
                                interval = args[1];
                                message.channel.send(`**✅ Automatic save has been enabled and set to ${ms(ms(interval), { long: true })}! To stop, type \`q!save stop\`**`);
                            }
                            // interval = args[1];
                        }
                        else {
                            interval = '15m';
                            message.channel.send(`**✅ Automatic save has been enabled and set to default value of ${ms(ms(interval), { long: true })}! To stop, type \`q!save stop\`**`);
                        }
