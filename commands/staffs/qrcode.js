...
module.exports = {
    ...
    async run(bot, message, args, prefix, cmd) {

        const toQR = args.join(" ");

        if (!toQR) {
            return message.channel.send("**❌ Please put a value to generate!**");
        }
        ...
        qrcode.toDataURL().then(async (data) => {
            ...
            await message.channel.send(`✅ **QR Code for \`${toQR}\` successfully generated!**`);
