module.exports = {
    ...
    run: async (bot, message) => {
        ...
        if (bot.queue.length === 0) {
            return message
                .reply(
                    "🙁 **The queue is empty! We can't start with an empty list!**"
                )
                .then((m) =>
                    m.delete({ timeout: 10000, reason: "It had to be done" })
                );
        }
        if (bot.queue.length > 0 && member.roles.cache.has(dj.id)) {
            return message.channel
                .send(
                    `🎉 **Let's begin! For our first performer, give it up for ${performer}! 👏👏👏**`
                )
                .then(performer.roles.add(role));
        }
        if (!member.roles.cache.has(dj.id)) {
            return message.reply(
                `🛑 **You don't have the \`${dj.name}\` role!**`
            );
        }
    },
};
